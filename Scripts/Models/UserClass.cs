...
namespace NT106.Scripts.Models
{
	public static class UserClass
	{
		...
		public static async Task<(bool, string)> RegisterAsync(string username, string email, string password, string confirm)
		{
			try
			{
				...
				if (!res.IsSuccessStatusCode)
					throw new Exception("Không thể đăng ký! Email có thể đã được sử dụng (hoặc mật khẩu quá yếu - ít nhất 6 ký tự).");
				...
				try
				{
					// Yêu cầu Cloudinary "copy" ảnh default và lưu lại với tên là "avatar/newUid"
					await CloudinaryService.CopyDefaultAvatarAsync(newUid);
				}
				catch (Exception cloudinaryEx)
				{
					// Nếu Cloudinary lỗi, cũng không sao, chỉ ghi log
					// Việc đăng ký vẫn được coi là thành công
					GD.PrintErr($"Lỗi set avatar default cho {newUid}: {cloudinaryEx.Message}");
				}
		...
		{
			// Hàm UploadImage của CloudinaryService đã bao gồm việc xóa ảnh cũ
